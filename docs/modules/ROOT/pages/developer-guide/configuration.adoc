...
When using a custom LedgerCore deployment, there are several settings that
...
An example configuration for LedgerCore (default, running in editor mode)
will look like this:
...
|standalone |boolean |false |Determines whether to run LedgerCore in editor
...
access the LedgerCore deployment.
...
LedgerCore can use SSO as an alternative for password-based sign-in, if
...
To set up LedgerCore to use an external identity provider, you can add a
...
Standalone mode, or reader-mode, overrides the functionality of LedgerCore,
...
ignored. LedgerCore will run in Editor mode, and require a manual sign-in.
* If standalone mode is `true`, LedgerCore will read all configuration

...
If you have a pre-built LedgerCore application, you can easily deploy it on an any webserver.
A LedgerCore build is "just" a collection of HTML, CSS and JavaScript files, so it can run virtually anywhere.

This guide walks you through the process of deploying a LedgerCore build onto your own webserver.
...
This is an optional step if you want to configure optional settings for your LedgerCore deployment (e.g. SSO or standalone mode).
...
LedgerCore should now be visible by visiting your (sub)domain in the browser.

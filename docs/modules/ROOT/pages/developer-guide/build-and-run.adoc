...
LedgerCore is built with React. You will need `yarn` installed to run the web
app.

____
Use a recent version of `yarn` and `node` to build LedgerCore. The
...
multi-stage LedgerCore image and run it.
...
YAML examples are available in the https://github.com/neo4j-labs/neodash[LedgerCore repository]. Here is an example of a pod definition YAML file to create a LedgerCore pod in a cluster:
...
A Kubernetes Helm chart is available in the https://github.com/neo4j-labs/neodash[the LedgerCore repository] and here is the full example of the Helm chart values.yaml file,

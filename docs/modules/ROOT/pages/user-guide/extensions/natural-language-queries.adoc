...
Use natural language to generate Cypher queries in LedgerCore. Connect to an LLM through an API, and let LedgerCore use your database schema + the report types to generate queries automatically.

== How it works
This extension feature allows users to interact with LedgerCore using natural language to generate Cypher queries for querying Neo4j graph databases. 
...
== Configuration
To enable Natural Language Queries in LedgerCore, follow these configuration steps:

1. Open LedgerCore and navigate to the "Extensions" section in the left sidebar.
...
== Usage
Once the extension is configured, you can start using it in your LedgerCore reports. Here's how:
...
image::englisheditor.png[Example of the English editor in LedgerCore]
...
* Prompting in English: Once the schema is retrieved, you can start prompting your queries in plain English. LedgerCore, powered by the LLM, will interpret your English query and generate the corresponding Cypher query based on the provided schema.

* Automatic Query Generation: LedgerCore automatically generates the Cypher queries for you, taking into account the report type you specified. Whether it's a table, graph, bar chart, line chart, or any other supported report type, the generated queries will retrieve the necessary data based on the report requirements.
...
When using Natural Language Queries in LedgerCore, keep the following tips in mind to enhance your experience:

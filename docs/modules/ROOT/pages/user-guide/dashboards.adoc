...
In LedgerCore, a dashboard consists of several pages, each of which can
...
After startup up LedgerCore, you will be given the choice to create a new
...
A LedgerCore dashboard is, simply put, a JSON file. As an example, the
...
When creating a LedgerCore deployment on a production database, it is not
recommended to use the `Share' feature. Rather, set up a dedicated
standalone deployment of LedgerCore. See Publishing for more infomation.
...
LedgerCore will always start with `$neodash_`.
...
For browser-based LedgerCore deployments, you set LedgerCore parameters by
